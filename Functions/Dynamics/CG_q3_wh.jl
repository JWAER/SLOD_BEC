function CG_q3_ωₕ(U0,T,Nt,ϕ,∇φᵢ∇φⱼ,φᵢφⱼ,Vφᵢφⱼ,ω,TOL,max_it,β,ϵ,save_it,save_here)
...
#  tid1 = time();      
        ...
        Ûₕ = ϕ'*Û;
        ρ = φᵢφⱼ_lu\(ϕ*Assemble.ρvᵢ(Ûₕ,ωₕ)) 
        ρₕ = ϕ'*ρ;
...
function CG_q3_ωₕ1(U0,T,Nt,∇φᵢ∇φⱼ,φᵢφⱼ,Vφᵢφⱼ,ω,TOL,max_it,β,ϵ,save_it,save_here)
# Lagrage FEM of order p for spatial discretization
k = T/Nt;
## numerical parameter
...
function CG_q3_ωₕ_IterativeSolvers(U0,T,Nt,ϕ,∇φᵢ∇φⱼ,φᵢφⱼ,Vφᵢφⱼ,ω,TOL,max_it,β,ϵ,save_it,save_here)
# Lagrage FEM of order p for spatial discretization
k = T/Nt;
## numerical parameter
...
for n = 1:Nt
 ...
 tid1 = time();      
        Û = c0[1]*U+c[1,1]*ξⁱ_1+c[2,1]*ξⁱ_2+c[3,1]*ξⁱ_3;
        Ûₕ = ϕ'*Û;
        ρ = cg(φᵢφⱼ,ϕ*Assemble.ρvᵢ(Ûₕ,ωₕ)) 
        ρₕ = ϕ'*ρ;
        f₁ = ϕ*Assemble.u²uvᵢ(ωₕ,Ûₕ,ρₕ)
  ...
  tid2 = time();
        ...
        ξⁱ⁺¹_1 = idrs(M1,F1,reltol=10^-11);
        ξⁱ⁺¹_2 = idrs(M2,F2,reltol=10^-11);
        ξⁱ⁺¹_3 = idrs(M3,F3,reltol=10^-11);
   ...
   println("one time step ", time()-tid, " it ", it)
